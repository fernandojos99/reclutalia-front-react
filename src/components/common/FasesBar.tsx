...
export function FasesBar({ v, compact, timeline, activo = null, onSub }: Props) {
  ...
  const [abierto, setAbierto] = useState(false); // vista interactiva: pasos colapsados por defecto
  ...
  return (
    <div className="fases-panel">
      {/* Cabecera siempre visible (pills de las 3 fases + etapa actual) con flecha para abrir/cerrar los pasos */}
      <div className="fases-panel-hd">
        <div className="fases-c">
          {FASES.map((f, i) => {
            const st = done || i + 1 < fase ? " done" : i + 1 === fase ? " now" : "";
            return (
